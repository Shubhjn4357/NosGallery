...
class NOSHealthBreathWidget : NosBaseWidgetProvider() {
    ...
    override fun onUpdateWidget(
        ...
        val title = customs?.optString("titleText") ?: "BREATHING PACER"
        container.addView(R.id.dynamic_view_container, createHeader(context, title, "heart", accentColor, subtextColor))
        
        container.addView(R.id.dynamic_view_container, createTextView(context, "INHALE... EXHALE", 16f, textColor, marginTop = 6f))

        val btnBg = if (theme == "minimal") android.graphics.Color.parseColor("#ffe0e0e0") else android.graphics.Color.parseColor("#ff1c1c1e")
        container.addView(R.id.dynamic_view_container, createButtonView(context, "START PACER", "toggle_breath", appWidgetId, textColor, btnBg))
