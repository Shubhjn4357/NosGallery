 ...
 * by the host application. The native side is a pure rendering engine.
 */
open class NosBaseWidgetProvider : AppWidgetProvider() {
     ...
     * to determine what state mutation to perform.
     */
    ...
    private fun updateWidget(
        ...
        val customViews = onUpdateWidget(context, appWidgetManager, appWidgetId, config, theme)
        if (customViews != null) {
            appWidgetManager.updateAppWidget(appWidgetId, customViews)
            return
        }
    ...
    fun createRootView(context: Context, theme: String, customs: JSONObject?): RemoteViews {
        val views = RemoteViews(context.packageName, R.layout.nos_widget_layout)
        views.removeAllViews(R.id.nos_widget_root)
        val bgColor = parseColorOr(customs?.optString("backgroundColor"), themeBackground(theme))
        views.setInt(R.id.nos_widget_root, "setBackgroundColor", bgColor)
    ...
    fun createHeader(context: Context, title: String, iconName: String, accentColor: Int, subtextColor: Int): RemoteViews {
        val row = createRowView(context)
        
        val img = createImageView(context, iconName, accentColor)
        row.addView(R.id.dynamic_view_container, img)

        val txt = createTextView(context, title.uppercase(Locale.getDefault()), 8f, subtextColor)
        row.addView(R.id.dynamic_view_container, txt)

        return row
    }

    fun createFooter(context: Context, text: String, accentColor: Int): RemoteViews {
        val txt = createTextView(context, text.uppercase(Locale.getDefault()), 7.5f, accentColor, marginTop = 6f)
        return txt
    }

    fun createTextView(context: Context, text: String, fontSizeSp: Float, color: Int, marginTop: Float = 0f, isWeight: Boolean = false): RemoteViews {
        val layoutRes = if (isWeight) R.layout.widget_dynamic_text_weight else R.layout.widget_dynamic_text
        val views = RemoteViews(context.packageName, layoutRes)
        views.setTextViewText(R.id.dynamic_text, text)
        views.setTextViewTextSize(R.id.dynamic_text, android.util.TypedValue.COMPLEX_UNIT_SP, fontSizeSp)
        views.setTextColor(R.id.dynamic_text, color)
        
        if (marginTop > 0f && android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.S) {
            views.setViewLayoutMargin(R.id.dynamic_text, RemoteViews.MARGIN_TOP, marginTop, android.util.TypedValue.COMPLEX_UNIT_DIP)
        }
        return views
    }

    fun createImageView(context: Context, iconName: String, color: Int, widthDp: Float = 10f, heightDp: Float = 10f): RemoteViews {
        val views = RemoteViews(context.packageName, R.layout.widget_dynamic_image)
        val resName = "ic_" + iconName.lowercase(Locale.getDefault())
        val resId = context.resources.getIdentifier(resName, "drawable", context.packageName)
        if (resId != 0) {
            views.setImageViewResource(R.id.dynamic_image, resId)
            if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.S) {
                views.setColorStateList(R.id.dynamic_image, "setImageTintList", android.content.res.ColorStateList.valueOf(color))
            } else {
                views.setInt(R.id.dynamic_image, "setColorFilter", color)
            }
        }
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.S) {
            views.setViewLayoutWidth(R.id.dynamic_image, widthDp, android.util.TypedValue.COMPLEX_UNIT_DIP)
            views.setViewLayoutHeight(R.id.dynamic_image, heightDp, android.util.TypedValue.COMPLEX_UNIT_DIP)
        }
        return views
    }

    fun createProgressBar(context: Context, value: Int, max: Int, color: Int): RemoteViews {
        val views = RemoteViews(context.packageName, R.layout.widget_dynamic_progress)
        views.setProgressBar(R.id.dynamic_progress, max, value, false)
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.S) {
            views.setColorStateList(R.id.dynamic_progress, "setProgressTintList", android.content.res.ColorStateList.valueOf(color))
            views.setViewLayoutHeight(R.id.dynamic_progress, 6f, android.util.TypedValue.COMPLEX_UNIT_DIP)
            views.setViewLayoutWidth(R.id.dynamic_progress, 60f, android.util.TypedValue.COMPLEX_UNIT_DIP)
            views.setViewLayoutMargin(R.id.dynamic_progress, RemoteViews.MARGIN_TOP, 6f, android.util.TypedValue.COMPLEX_UNIT_DIP)
        }
        return views
    }

    fun createButtonView(context: Context, label: String, action: String, appWidgetId: Int, textColor: Int, bgColor: Int): RemoteViews {
    ...
    fun createClockView(context: Context, format: String, fontSizeSp: Float, color: Int, marginTop: Float = 0f): RemoteViews {
        val views = RemoteViews(context.packageName, R.layout.widget_dynamic_clock)
        views.setCharSequence(R.id.dynamic_clock, "setFormat12Hour", format)
        views.setCharSequence(R.id.dynamic_clock, "setFormat24Hour", if (format.contains("a")) "HH:mm" else format)
        views.setTextViewTextSize(R.id.dynamic_clock, android.util.TypedValue.COMPLEX_UNIT_SP, fontSizeSp)
        views.setTextColor(R.id.dynamic_clock, color)
        if (marginTop > 0f && android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.S) {
            views.setViewLayoutMargin(R.id.dynamic_clock, RemoteViews.MARGIN_TOP, marginTop, android.util.TypedValue.COMPLEX_UNIT_DIP)
        }
        return views
    }

    fun createChronometerView(context: Context, fontSizeSp: Float, color: Int, baseTime: Long, running: Boolean): RemoteViews {
        val views = RemoteViews(context.packageName, R.layout.widget_dynamic_chronometer)
        views.setTextViewTextSize(R.id.dynamic_chronometer, android.util.TypedValue.COMPLEX_UNIT_SP, fontSizeSp)
        views.setTextColor(R.id.dynamic_chronometer, color)
        views.setChronometer(R.id.dynamic_chronometer, baseTime, null, running)
        return views
    }

    fun createRowView(context: Context, gap: Float = 0f): RemoteViews {
        val views = RemoteViews(context.packageName, R.layout.widget_dynamic_view_row)
        return views
    }

    fun createColumnView(context: Context, gap: Float = 0f): RemoteViews {
        val views = RemoteViews(context.packageName, R.layout.widget_dynamic_view_column)
        return views
    }
}
