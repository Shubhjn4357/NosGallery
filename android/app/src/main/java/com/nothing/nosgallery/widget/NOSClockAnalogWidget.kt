...
class NOSClockAnalogWidget : NosBaseWidgetProvider() {
    ...
    override fun onUpdateWidget(
        ...
        val title = customs?.optString("titleText") ?: "ANALOG CLOCK"
        container.addView(R.id.dynamic_view_container, createHeader(context, title, "clock", accentColor, subtextColor))
        
        val valTxt = customs?.optString("valueText") ?: "Ticking..."
        container.addView(R.id.dynamic_view_container, createTextView(context, valTxt, 22f, textColor, marginTop = 6f))
        
        val sub = customs?.optString("subValueText") ?: "CLASSIC DIAL"
