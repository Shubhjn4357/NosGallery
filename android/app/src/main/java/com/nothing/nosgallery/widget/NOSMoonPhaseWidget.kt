...
class NOSMoonPhaseWidget : NosBaseWidgetProvider() {
    ...
    override fun onUpdateWidget(
        ...
        val title = customs?.optString("titleText") ?: "MOON PHASE"
        container.addView(R.id.dynamic_view_container, createHeader(context, title, "moon", accentColor, subtextColor))
        
        val valTxt = customs?.optString("valueText") ?: "GIBBOUS"
        container.addView(R.id.dynamic_view_container, createTextView(context, valTxt, 18f, textColor, marginTop = 6f))
        
        val sub = customs?.optString("subValueText") ?: "85% ILLUMINATION"
        container.addView(R.id.dynamic_view_container, createTextView(context, sub, 8f, subtextColor, marginTop = 4f))
        
        val footer = customs?.optString("footerText") ?: "NOS • MOON"
