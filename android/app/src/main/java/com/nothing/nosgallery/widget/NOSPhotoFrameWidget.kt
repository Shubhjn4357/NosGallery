...
class NOSPhotoFrameWidget : NosBaseWidgetProvider() {
    ...
    override fun onUpdateWidget(
        ...
        val title = customs?.optString("titleText") ?: "PHOTO FRAME"
        container.addView(R.id.dynamic_view_container, createHeader(context, title, "camera", accentColor, subtextColor))
        
        container.addView(R.id.dynamic_view_container, createTextView(context, "No Image Selected", 16f, textColor, marginTop = 6f))
        
        val sub = customs?.optString("subValueText") ?: "TAP TO SELECT"
        container.addView(R.id.dynamic_view_container, createTextView(context, sub, 8f, subtextColor, marginTop = 4f))
        
        val footer = customs?.optString("footerText") ?: "NOS • GALLERY"
